		...
		for(long long  i = 1 ; i <= A.numRows() ;i++)
		{
			for(long long  j = 1; j <= A.numCols() ; j++)
			{
				(i+j % 2 == 0) ? sign = 1 : sign = -1;
					...
					R(i,j) = determinat(temp.removeCol(j).removeRow(i)) * sign; 
			}
		}
		return R; 
	}
...
{
	return inverseGuassJordan( (tranpose(A) * A) ) * tranpose(A);
}

matrix<double> la_pack::inversePseudoQR(matrix<double> A)// NOT SURE ABOUT CORRECTNESS
{
	matrix<double> QR = QRFactorizeCol(A);
	return inverseGuassJordan(getR_from_QR(QR)) * tranpose(getQ_from_QR(QR));
}
...
matrix<double> la_pack::regularizationTikonon(matrix<double> A, double alpha)
{
	...
	return inverseGuassJordan((tranpose(A) * A) - alphaI)*tranpose(A);
}
...
double la_pack::innerProduct(matrix<double> &A, matrix<double> &B)
/*
	inner Product :
	At * B // t = transpose
	Row Vec * Col Vec 
...
matrix<double> la_pack::gramSchmidtOrtho(matrix<double> A,bool normalize)
{
	...
	for(int i = 1, j = 1 ; i <= A.numRows() ;i++,j++)  // No operation to be done on row 1 , We make over rows orthogonal to row 1
	{
		...
		for(int q = j ; q >= 1 ; q--) // Moves from current row to top
		{
			if(q!=i)
			{
				residue = residue + projectionOn(A.returnRow(i),A.returnRow(q));   ///DEBUG
			}														
		}
...
matrix<double> la_pack::QRFactorizeCol(matrix<double> A, bool normalize) // NOT SURE ABOUT CORRECTNESS
{
	...
	for (int i = 1; i <= A.numCols(); i++)  // No operation to be done on COl 1 , We make over cols orthogonal to col 1
	{
		...
		for (int q = i; q >= 1; q--) // Moves from current Col to first
		{
			...
			if (q != i)
			{
				upper = innerProduct(A.returnCol(i), A.returnCol(q));
				lower = innerProduct(A.returnCol(q), A.returnCol(q));
				multiplier = upper / lower;
				R(q,i) = multiplier;
		...
		if (normalize)
		{
			...
			R(i, i) = innerProduct(A.returnCol(i), unit);
...
matDouble la_pack::createBlockColVec_2(matDouble A, matDouble B)
{
	if (A.numCols() == B.numCols())
	{
		int Arows = A.numRows(), Acols = A.numCols();
		matDouble result(A.numRows() + B.numRows(), A.numCols());
...
matDouble la_pack::createBlockRowVec_2(matDouble A, matDouble B)
{
	if (A.numRows() == B.numRows())
	{
		int Arows = A.numRows(), Acols = A.numCols();
		matDouble result(A.numRows(), A.numCols() + B.numCols());
		for (long long i = 1; i <= result.numRows(); i++)
		{
			for (long long k = 1; k <= result.numCols(); k++)
			{
...
matDouble la_pack::createBlockSquare_4(matDouble A, matDouble B, matDouble C, matDouble D)
{
	// ERROR HANDLING 
	if (A.numCols() != B.numCols())
	{
	...
	else
	{
		...
		matDouble result(A.numRows() + B.numRows(), A.numCols() + C.numCols());
		int Arows = A.numRows(), Acols = A.numCols();
		for (long long i = 1; i <= result.numRows(); i++)
		{
			for (long long k = 1; k <= result.numCols(); k++)
...
void la_pack::gaussJordan(matDouble &A, matDouble &B)
{
	int iCol, iRow;
...
{
	matDouble B(A.numRows(), 0);
	gaussJordan(A, B);
}


matrix <double> la_pack::decomposeLU(matDouble &A)
{
	int nARows = A.numRows(); int nACols = A.numCols();
	matDouble joinedLU = A;// The matrix will have both the L and U combined .  
	matInt pivotIndex(A.numRows(),1);// Stores the pivot permutations
	matDouble implicitScaling(A.numRows(),1);
...
std::tuple<matDouble, matInt> la_pack::decomposeLU(matDouble &A, bool returnPivot)
{
	int nARows = A.numRows(); int nACols = A.numCols();
	matDouble joinedLU = A;// The matrix will have both the L and U combined .  
	matInt pivotIndex(A.numRows(), 1);// Stores the pivot permutations
	...
	std::tuple<matDouble, matInt> result(joinedLU,pivotIndex);
	return result; 
}


matrix <double> la_pack::solveLU(matDouble &A, matDouble &B )
{
	int nARows = A.numRows();
	int nonZero = 0; //  First Element of B , which is not nonZERO 
	auto LU_P = decomposeLU(A,true);
	matDouble LUm = std::get<0>(LU_P);
	matInt pivot = std::get<1>(LU_P);
	matDouble X(nARows, 1);
...
matrix <double> la_pack::solveLU(matDouble &A, matDouble &B, matDouble &LU, matInt pivot)
{
	...
	//matDouble LUm = LU; // std::get<0>(LU_P);
	//matInt pivot = pivotIndex; //  std::get<1>(LU_P);  // No need to create new matrices !  // Learn to conserve memory ! 
	matDouble X(nARows, 1);
...
matrix <double> la_pack::solveLU(matDouble &A, matDouble &B, int numRightSides)  
{
	int nEqns = numRightSides;
	int nARows = A.numRows();
	int nBRows = B.numRows();
	int nBCols = B.numCols();
	matDouble X(nARows,nBCols);
	...
	matDouble tempCol(nBRows,1); // Used to store the columns of B one by one 
...
matrix<double> la_pack::inverseLU(matDouble & A)
{
	matDouble R = getIdentity(A.numRows());
